            ...
            while True:
                if Scheduler._bFlag:
                    ...
                    if filename:
                        ...
                        logger.debug('get queue msg : %s, queue-size: %d' % (filename, QueueUtils.get_queue().qsize()))
                        array_list = []
                        ...
                        try:
                            # 上传文件到宇视的ftp服务器
                            ftpPath = VehPass.filename_format(os.path.basename(filename))
                            ...
                            logger.error('ftp upload failed %s, filename: %s' % (e.message, array_list))
